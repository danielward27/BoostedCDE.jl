...
@testset "predict, step! and boost!" begin
    ...
    @test ϕₘ ≈ predict(model, θ, x)  # Accumulated during training vs predict from scratch
...
Fit with random data, and see if it can predict above example approximately correct?